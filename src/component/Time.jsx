import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { faArrowLeft } from '@fortawesome/free-solid-svg-icons';

import "./Details.css"
import "./Time.css"
import "./Booking.css"

import { useState } from "react";
import { useNavigate } from "react-router-dom";

function Time() {
    let time = [
        { id: 1, time: "11:45 AM" },
        { id: 2, time: "07:22 PM" },
        { id: 3, time: "03:16 AM" },
        { id: 4, time: "02:59 PM" },
        { id: 5, time: "12:33 PM" },
        { id: 6, time: "05:07 AM" },
        { id: 7, time: "10:41 PM" },
        { id: 8, time: "01:18 AM" },
        { id: 9, time: "04:30 PM" },
        { id: 10, time: "09:50 AM" },
        { id: 1, time: "11:45 AM" },
        { id: 2, time: "07:22 PM" },
        { id: 3, time: "03:16 AM" },
        { id: 4, time: "02:59 PM" },
        { id: 5, time: "12:33 PM" },
        { id: 6, time: "05:07 AM" },
        { id: 7, time: "10:41 PM" },
        { id: 8, time: "01:18 AM" },
        { id: 9, time: "04:30 PM" },
        { id: 10, time: "09:50 AM" }
    ]
    const [selectedTime, setSelectedTime] = useState([time[10]]);
    const [schedule, setSchedule] = useState("");

    const navigate = useNavigate();

    function handlePopup() {
        let popUp = document.querySelector(".pop-up");
        let fullpopUp = document.querySelector(".full-pop-up");
        popUp.style.display = "flex";
        fullpopUp.style.display = "flex";
        popUp.style.flexDirection = "column";
    }

    function validateForm(event) {
        event.preventDefault();
        let variable = document.forms["myForm"]["Demail"].value;
        console.log(variable);
        if (variable === "") {
            alert("Please enter a valid email address");
        } else {
            // Show the next step of the form
            let popupdata = document.querySelector(".pop-up-data");
            let continuepopup = document.querySelector(".continue-pop-up");
            popupdata.style.display = "none";
            continuepopup.style.display = "flex";
        }
    }

    function validateAccountform(e) {
        e.preventDefault();
        let fname = document.forms["account-form"]["fname"].value;
        if (fname === "") {
            alert("enter your first name");
        }
        else {
            console.log("iuerfu");

        }
    }

    function removePopup() {
        let popUp = document.querySelector(".pop-up");
        let fullpopUp = document.querySelector(".full-pop-up");

        popUp.style.display = "none";
        fullpopUp.style.display = "none";

    }
    function pickSchedule(e) {
        let toppart2 = document.querySelector(".top-part2");
        toppart2.style.display = "none";
        console.log(e.target.children[0].textContent);
        setSchedule(e.target.children[0].textContent);

    }

    function taketoFirstvisit() {
        navigate("/First_visit_check");
    }


    let month = [
        { number: 1, weekday: "Mon" },
        { number: 2, weekday: "Tues" },
        { number: 3, weekday: "Wedn" },
        { number: 4, weekday: "Thu" },
        { number: 5, weekday: "Fri" },
        { number: 6, weekday: "Sat" },
        { number: 7, weekday: "Sun" },
        { number: 8, weekday: "Mon" },
        { number: 9, weekday: "Tues" },

        { number: 10, weekday: "Mon" },
        { number: 11, weekday: "Tues" },
        { number: 12, weekday: "Wedn" },
        { number: 1, weekday: "Mon" },
        { number: 2, weekday: "Tues" },
        { number: 3, weekday: "Wedn" },
        { number: 4, weekday: "Thu" },
        { number: 5, weekday: "Fri" },
        { number: 6, weekday: "Sat" },
        { number: 7, weekday: "Sun" },
        { number: 8, weekday: "Mon" },
        { number: 9, weekday: "Tues" },

        { number: 10, weekday: "Mon" },
        { number: 11, weekday: "Tues" },
        { number: 12, weekday: "Wedn" },
        { number: 1, weekday: "Mon" },
        { number: 2, weekday: "Tues" },
        { number: 3, weekday: "Wedn" },
        { number: 4, weekday: "Thu" },
        { number: 5, weekday: "Fri" },
        { number: 6, weekday: "Sat" },
        { number: 7, weekday: "Sun" },
        { number: 8, weekday: "Mon" },
        { number: 9, weekday: "Tues" },

        { number: 10, weekday: "Mon" },
        { number: 11, weekday: "Tues" },
        { number: 12, weekday: "Wedn" },
        { number: 1, weekday: "Mon" },
        { number: 2, weekday: "Tues" },
        { number: 3, weekday: "Wedn" },
        { number: 4, weekday: "Thu" },
        { number: 5, weekday: "Fri" },
        { number: 6, weekday: "Sat" },
        { number: 7, weekday: "Sun" },
        { number: 8, weekday: "Mon" },
        { number: 9, weekday: "Tues" },

        { number: 10, weekday: "Mon" },
        { number: 11, weekday: "Tues" },
        { number: 12, weekday: "Wedn" },
        { number: 4, weekday: "Thursday" },
        { number: 5, weekday: "Friday" },
        { number: 6, weekday: "Saturday" },
        { number: 7, weekday: "Sunday" },
        { number: 8, weekday: "Monday" },
        { number: 9, weekday: "Tuesday" },

        { number: 1, weekday: "Monday" },
        { number: 2, weekday: "Tuesday" },
        { number: 3, weekday: "Wednesday" },
        { number: 4, weekday: "Thursday" },
        { number: 5, weekday: "Friday" },
        { number: 6, weekday: "Saturday" },
        { number: 7, weekday: "Sunday" },
        { number: 8, weekday: "Monday" },
        { number: 9, weekday: "Tuesday" },

        { number: 30, weekday: "Wednesday" }
    ]

    function moveRoot() {
        navigate("/Booking");
    }

    return (
        <>
            <div id="mainDetail" >
                <div className="absolute" onClick={moveRoot}>
                    <FontAwesomeIcon icon={faArrowLeft} size="2x" style={{ color: 'dark' }} />
                </div>
                <div className="container" id="container">

                    {/* ==============shops=============== */}

                    <div className="item1" id="item1">
                        <h2 className="">Select Time</h2>
                        <div className="header" id="time-header" >
                            {
                                month.map((val, index) => (
                                    <div className="month"
                                        key={index}
                                        onClick={() => setSelectedTime(time[index])}
                                    >
                                        <p className="month-date">{val.number}</p>
                                        <p className="month-day">{val.weekday}</p>
                                    </div>
                                ))
                            }
                        </div>
                        <div className="time-parent" id="schedule">
                            {selectedTime && (
                                <div onClick={pickSchedule}>
                                    <div className="time-card" key={selectedTime.id}>
                                        <div className="time-map">
                                            <p className="time-data">{selectedTime.time}</p>
                                        </div>
                                    </div>
                                    <div className="time-card" key={selectedTime.id}>
                                        <div className="time-map">
                                            <p className="time-data">{selectedTime.time}</p>
                                        </div>
                                    </div>

                                </div>
                            )}
                        </div>
                    </div>

                    {/* ==============services=============== */}

                    <div className="selected-service">
                        <div className="selected-service-div">
                            <div className="top-part">
                                <div className="top-part1">
                                    <div>
                                        <img src="data:image/jpeg;base64," alt="" className="top-img" />

                                    </div>
                                    <div>
                                        <p className="top-para">Arealyou Premium Salon</p>
                                        <p className="top-para">4.7 </p>
                                        <p>Hiran Magri, Siva Nagar, Udaipur</p>
                                    </div>
                                </div>
                                <div className="top-part2">No services selected</div>
                                <div>{schedule}</div>
                            </div>
                            <hr />
                            <div className="bottom-part">
                                <div className="top-bottom top-para">
                                    <p>Total</p>
                                    <p>free</p>
                                </div>
                                <div className="bottom-bottom">
                                    <button className="contactButton" onClick={handlePopup}>
                                        Continue
                                        <div className="iconButton">
                                            <svg
                                                height="24"
                                                width="24"
                                                viewBox="0 0 24 24"
                                                xmlns="http://www.w3.org/2000/svg"
                                            >
                                                <path d="M0 0h24v24H0z" fill="none"></path>
                                                <path
                                                    d="M16.172 11l-5.364-5.364 1.414-1.414L20 12l-7.778 7.778-1.414-1.414L16.172 13H4v-2z"
                                                    fill="currentColor"
                                                ></path>
                                            </svg>
                                        </div>
                                    </button>
                                </div>
                            </div>
                        </div>
                    </div>

                    {/* ==============popup=============== */}

                    <div className="full-pop-up" >
                        <div className="pop-up">
                            <span className="close-pop-up" onClick={removePopup}>&times;</span>
                            <hr />
                            <div className="pop-up-data">
                                <h1>Login or sign up</h1>
                                <h2>Log in or sign up to complete your booking</h2>
                                <a href="https://www.facebook.com/login.php?skip_api_login=1&api_key=1224875000951200&kid_directed_site=0&app_id=1224875000951200&signed_next=1&next=https%3A%2F%2Fwww.facebook.com%2Fv3.3%2Fdialog%2Foauth%3Fapp_id%3D1224875000951200%26cbt%3D1726649024465%26channel_url%3Dhttps%253A%252F%252Fstaticxx.facebook.com%252Fx%252Fconnect%252Fxd_arbiter%252F%253Fversion%253D46%2523cb%253Dfc8cc4ce0f0dcfb7f%2526domain%253Dwww.fresha.com%2526is_canvas%253Dfalse%2526origin%253Dhttps%25253A%25252F%25252Fwww.fresha.com%25252Ff595c74627b5eeebc%2526relation%253Dopener%26client_id%3D1224875000951200%26display%3Dpopup%26domain%3Dwww.fresha.com%26e2e%3D%257B%257D%26fallback_redirect_uri%3Dhttps%253A%252F%252Fwww.fresha.com%252Fa%252Ftrimmings-salon-spa-dempsey-hill-singapore-75b-loewen-road-lhclsrrf%252Fbooking%252Ftime%253FofferItems%253Dsv%25253A5701427%2523modal-authflow%26locale%3Den_US%26logger_id%3Dff4361506e3c63c07%26origin%3D1%26redirect_uri%3Dhttps%253A%252F%252Fstaticxx.facebook.com%252Fx%252Fconnect%252Fxd_arbiter%252F%253Fversion%253D46%2523cb%253Df02eb283c5dc1f669%2526domain%253Dwww.fresha.com%2526is_canvas%253Dfalse%2526origin%253Dhttps%25253A%25252F%25252Fwww.fresha.com%25252Ff595c74627b5eeebc%2526relation%253Dopener%2526frame%253Df14416627ec5876a4%26response_type%3Dtoken%252Csigned_request%252Cgraph_domain%26scope%3Dpublic_profile%252Cemail%26sdk%3Djoey%26version%3Dv3.3%26ret%3Dlogin%26fbapp_pres%3D0%26tp%3Dunspecified&cancel_url=https%3A%2F%2Fstaticxx.facebook.com%2Fx%2Fconnect%2Fxd_arbiter%2F%3Fversion%3D46%23cb%3Df02eb283c5dc1f669%26domain%3Dwww.fresha.com%26is_canvas%3Dfalse%26origin%3Dhttps%253A%252F%252Fwww.fresha.com%252Ff595c74627b5eeebc%26relation%3Dopener%26frame%3Df14416627ec5876a4%26error%3Daccess_denied%26error_code%3D200%26error_description%3DPermissions%2Berror%26error_reason%3Duser_denied&display=popup&locale=en_GB&pl_dbl=0" className="first">
                                    <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQUcP0ZcWRME2hXax1sPPgNtutzs7H0ZQv2vw&s" alt="" className="h-7 w-7" />
                                    Continue with Facebook</a>
                                <a href="https://accounts.google.com/o/oauth2/auth/oauthchooseaccount?redirect_uri=storagerelay%3A%2F%2Fhttps%2Fwww.fresha.com%3Fid%3Dauth728840&response_type=permission%20id_token&scope=email%20profile%20openid%20https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fuser.gender.read%20https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fuser.birthday.read&openid.realm&include_granted_scopes=true&client_id=939324003214-fq16j74hi8jl19u6uhfe0b5l6o5nu7q9.apps.googleusercontent.com&ss_domain=https%3A%2F%2Fwww.fresha.com&fetch_basic_profile=true&gsiwebsdk=2&service=lso&o2v=1&ddm=1&flowName=GeneralOAuthFlow" className="first">
                                    <img src="https://image.similarpng.com/very-thumbnail/2020/06/Logo-google-icon-PNG.png" className="w-7 h-77" />
                                    continue with google
                                </a>
                                <div className="division"><a className="hr"></a>OR<div className="hr"></div></div>

                                <form className="form" name="myForm" method="post" onSubmit={validateForm}>
                                    <input type="email" name="Demail" required placeholder="        Email Address" className="first" />

                                    <div className="first" id="btn-first">
                                        <input type="submit" value="Continue" className="text-white w-1/2" />
                                    </div>
                                </form>
                            </div>
                            <div className="continue-pop-up">
                                <h2>Create account</h2>
                                <p>Create your new account for
                                    <br />randomName123@gmail.com by completing these
                                    <br />details
                                </p>
                                <form className="form" method="post" name="account-form" onSubmit={validateAccountform}>
                                    <div className="name">
                                        <div >
                                            <label htmlFor="fname" className="font-bold">first name*</label>
                                            <input type="text" name="fname" id="fname" required className="border p-3 rounded-lg" />
                                        </div>
                                        <div >
                                            <label htmlFor="lname" className="font-bold">last name*</label>
                                            <input type="text" name="lname" id="lname" required className="border p-3 rounded-lg" />
                                        </div>
                                    </div>
                                    <div className="flex flex-col">
                                        <label htmlFor="pass" className="font-bold">password*</label>
                                        <input type="text" id="pass" required className="border p-3 rounded-lg" />
                                    </div>
                                    <div className="flex flex-col gap-3">
                                        <label htmlFor="mobile" className="font-bold">mobile number*</label>
                                        <div>
                                            <select name="mobile" className="w-1/5 p-3 rounded-lg border mr-3">
                                                <option value="">91</option>
                                                <option value="">91</option>
                                                <option value="">91</option>
                                                <option value="">91</option>
                                                <option value="">91</option>
                                            </select>
                                            <input type="text" id="mobile" required className="border p-3 w-3/4 rounded-lg" />
                                        </div>
                                    </div>
                                    <div>
                                        <input type="checkbox" required id="check1" className="" />
                                        <label htmlFor="check1" className="font-semibold ml-3">
                                            I agree to receive marketting notification with offers and news.
                                        </label>
                                    </div>
                                    <div>
                                        <input type="checkbox" required id="check2" />
                                        <label htmlFor="check2" className="font-semibold ml-3">I agree to the privacy policy,Term of use and Term of Service.</label>
                                    </div>

                                    <div className="btn-continue-popup" id="btn-first"><button type="submit" className="text-white" onClick={taketoFirstvisit} >Continue</button></div>

                                </form>
                            </div>
                        </div>
                    </div>
                </div >
            </div >



        </>
    )
}

export default Time